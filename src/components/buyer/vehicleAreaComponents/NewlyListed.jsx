...
import {get_new_listing} from '../../../actions/homeActions'
const options = {
    loop: true,
    rewind: true,
    margin: 14,
...
const NewlyListed = props => {
    const dispatch = useDispatch()
    const state = useSelector((state)=> {return {
        new_listing:state.homeReducer.new_listing,
        new_listing_loading:state.homeReducer.new_listing_loading,
    ...
    useEffect(() => {
        ...
        dispatch(get_new_listing(data))
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [])
    return (<React.Fragment>
        {state.new_listing_loading ? (<><OwlCarousel options={owlOptions} className="owl-one owl-carousel owl-theme"><HomeNewlyListingPlaceHolder /><HomeNewlyListingPlaceHolder /><HomeNewlyListingPlaceHolder /><HomeNewlyListingPlaceHolder /> </OwlCarousel></>) : (
            (state.new_listing || []).length > 0 ?
                <OwlCarousel options={options} className="owl-one owl-carousel owl-theme">
                    {(state.new_listing || []).map((item, index) => (
                        ...
                        <Link to={{
                            ...
                            <div className="view-all-vihicles-in-area">
                                <div className="view-all-vihicles-in-area-inner">
                                    ...
                                    <p> Newly listed vehicles in your area </p>
                                </div>
                            </div>
                        </Link>
                    </div>
                </OwlCarousel> : <EmptyComponent name='Newly Listings' />)}
    </React.Fragment>)
}
export default memo(NewlyListed)