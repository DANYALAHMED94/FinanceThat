...
import { private_application_types, application_status } from '../../../../../constants';
    ...
    return (<>
        ...
        <div className="row">
            ...
            <MultiSelectDropDown dataOptions={[{ label: 'Applicant', value: 'applicant' },{value:'agent', label:"Agent"}]} selectedValue={applicationCreatedBy}
                ...
                {/* <MultiSelectDropDown dataOptions={[...createdBy_options, ...(agent_listing || []).filter(item => item?.aud_user_id && item?.aud_user_id?.length > 0).map((item) => {
                    return {
                        value:
                            item?.aud_user_id &&
