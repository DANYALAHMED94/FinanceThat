export const vehicleImages = (item) => {
    return (<>
                ...
                (item.name.toUpperCase() === ('Boat').toUpperCase() || item.name.toUpperCase() === ('Boats').toUpperCase() || item.name.toUpperCase() === ('Marine').toUpperCase()) ? <img src="/assets/image/boat.svg" alt={item.name} /> :
                    item.name.toUpperCase() === ('Trailer').toUpperCase() || item.name.toUpperCase() === ('Trailers').toUpperCase() ? <img src="/assets/image/trailer.svg" alt={item.name} /> :
                        item.name.toUpperCase() === ("Powersport").toUpperCase() || item.name.toUpperCase() === ("Powersports").toUpperCase() ? <img src="/assets/image/power-sport.svg" alt={item.name} /> :
                            (item.name.toUpperCase() === ("Small Equipment").toUpperCase() || item.name.toUpperCase() === ("Small Equipments").toUpperCase() || item.name.toUpperCase() === ("Lawn Tractor").toUpperCase()) ? <img src="/assets/image/small-epuipment.svg" alt={item.name} /> :item.name.toUpperCase() === ("Construction").toUpperCase() || item.name.toUpperCase() === ("Constructions").toUpperCase() ? <img src="/assets/image/skidSteer2.svg" alt={item.name} width="100px"
                                                        height="69px"/> : ''}
                                </span>
                                <h1>{item.name}</h1>
    </>)
...
export const subVehicleImagesCon = (subItem) => {
    return (<>
    <span className="checkadpost"></span>
                                <span className="icon-holder">
