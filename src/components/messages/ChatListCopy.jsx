...
class ChatList extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevState.searchFriend !== this.state.searchFriend && this.state.searchFriend) {
            console.log(this.state.searchFriend, 'this.state.searchFriend')
    ...
    render() {
        return (
            <React.Fragment>
                <div className="col-xl-3 col-lg-3 col-md-3 col-sm-12 col-12 pl-0" id='ChatLeft-Container'>
                    {/*  */}
                    { }
                                        ...
                                        <a onClick={() => this.openChat(item.id, item.message_by, item.list_id, item.message_to)}>
                                            <div className="ChatDropdown-Head" id="ChatDropdown-Head">
                                                ...
                                                <h1>{item.list_desc} <br />{localStorage.getItem('user_type') == 1 ? (<span>{'Last message'} {moment(item.time).fromNow() || ''}</span>) : null}
                                                    <br />
                                                    {localStorage.getItem('user_type') == 2 ? (<small>{item.user_image !== undefined && item.user_image !== null && item.user_image !== '' ? (<img src={`${API_URL}/media/${item.user_image}`} alt='' />) : (<img src="assets/image/chatimg-13.png" alt="" />)} <span>{item.sender_name}</span></small>) : null}
                                                </h1>
                                        ...
                                        <li className={item.id === this.state.selected ? 'active' : ''} key={index}>
                                            <a onClick={() => this.openChat(item.id, item.message_by, item.list_id, item.message_to)}>
                                                <div className="ChatDropdown-Head" id="ChatDropdown-Head">
                                                    ...
                                                    <h1>{item.list_desc} <br />{localStorage.getItem('user_type') == 1 ? (<span>{'Last message'} {moment(item.time).fromNow() || ''}</span>) : null}
                                                        <br />
                                                        {localStorage.getItem('user_type') == 2 ? (<small>{item.user_image ? (<img src={`${API_URL}/media/${item.user_image}`} alt='' />) : (item.lister_name ? <span className="chat-avtar">{`${item.lister_name.split(' ')[0] ? item.lister_name.split(' ')[0].charAt(0).toUpperCase() : ''}${item.lister_name.split(' ')[1] ? item.lister_name.split(' ')[1].charAt(0).toUpperCase() : ''}`}</span> : '')}
